...
class Solution {
	// partition function similar to quick sort, sorting in descending order
	public static int partition(int[] arr, int low,int high)
	{
		int pivot = arr[high], pivotloc = low;
		for (int i = low; i <= high; i++) {
			// inserting elements of greater value to the left of the pivot location
			if (arr[i] > pivot) {
				int temp = arr[i];
				arr[i] = arr[pivotloc];
				arr[pivotloc] = temp;
	...
	public static void main(String[] args)
	{
		...
		System.out.println(kPosition+"th largest element in the array : "+ kthLargest(arr, 0, n- 1,kPosition));
	    }
	}
}
